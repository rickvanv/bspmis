...
long P;
long N;
char inputfilepath[STRLEN];
//char outputfilepath[STRLEN];
...
void bsp_process_recvd_msgs(bool *Alive_edge, bool *Alive_vertex, long const *Adj, long const *destproc, long const *v1new, long const *Start, long const *v0, long *nalive, long const nedges){
    ...
    for (long i=0 ; i<nmessages; i++){
        ...
        if (tag == DEADVERTEX){
            if (Alive_vertex[v]) {
                if (v == 0){
                    printf("vertex 0 killed without being added to MIS\n");
                }
...
void bspmis(){
    ...
    long n, nz, *ia, *ja;
    double *weight;
    double suma= bspinput2triple(inputfilepath,&n,&nz,&ia,&ja,&weight);/* Sequential part */
    long maxops=0;
...
//    }
    ...
    /* Store number of rows in N variable for output printing */
    long *Npointer;
    Npointer = &N;
    (*Npointer) = nrows;
...
//    long nops= 0;    // number of elemental operations carried out
    ...
    for (long k = 0; k < ncols; k++) {
        if (rowvertex[k] != DUMMY)
            randval_cols[k] = randval_rows[rowvertex[k]];
        else
            randval_cols[k] = -1;
    }
    ...
    for (long e = nedges; e < nedges_tot; e++) {
        bsp_get(destproc[e - nedges], randval_rows, destvertex[e - nedges] * sizeof(long),
                &(randval_cols[janew[e]]),
                sizeof(long));
    }
    bsp_sync();
            ...
            for (long v = 0; v < nvertices; v++) {
                if (Alive_vertex[v]) {
                    ...
                    if (maxrandedge < nedges){ // set boolean value in case ties need to be broken
                        winstie = (v > rowvertex[column]); // based on rownumber when edge is local //TODO: check if this works by setting all 'random' values to one
                    }
...
//    printf("Local MIS at processor %ld :", s);
    ...
    sprintf(filename, "output%ld.txt", s);
    printedoutput = fopen(filename, "w");
    for (long k=0; k<miscount; k++){
//        printf("%ld, ", locmis[k]+1);
        fprintf(printedoutput, "%ld, ", locmis[k]+1);
    }
    fprintf(printedoutput, "\n");
    fclose(printedoutput);
    fflush(stdout);
...
//    if (s==0){
//        printf("Found a MIS in only %.6lf seconds.\n",time1-time0);
//        printf("File = %s", inputfilepath);
//        fflush(stdout);
//    }
    bsp_end();
}
...
int main(int argc, char **argv){
    ...
    /* Sequential part */
    printf("Insert the filename of the .mtx file\n");
    fflush(stdout);
    scanf("%s", &inputfilepath);
    printf("Enter the amount of processors you want to use\n");
    fflush(stdout);
    scanf("%ld", &P);
//    mtxfilepath = "/home/rick/CLionProjects/ParallelAlgorithms/mis/data/ca-GrQc/ca-GrQc.mtx";
//    write_distributed_mtx(inputfilepath);
    /* SPMD part */
    bspmis();

    /* Combine output */
    FILE *output;
    output = fopen("../output.txt", "w"); // TODO: change output filename
    for (long q=0; q<P; q++){
        FILE *locoutput;
        char locfilename[STRLEN];
        char locmis[3*N];
        sprintf(locfilename, "output%ld.txt", q);
        locoutput = fopen(locfilename, "r");
        fscanf(locoutput, "%[^\n]", locmis);
        fprintf(output, "%s", locmis);
        fclose(locoutput);
        remove(locfilename);
    }
    fclose(output);
